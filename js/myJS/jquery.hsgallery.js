...
Util.Drag.prototype = {
	...
	init : function(container,drag,options){
		...
		this.Move = hsg.bindAsEventListener(this,this.move);
		this.Stop = hsg.bindAsEventListener(this,this.stop);
		this.drag.style.cursor = "move";
		hsg.addEvent(this.drag,hsg.bindAsEventListener(this,this.start),"mousedown");
	},
	start : function(E){
		this.x = E.clientX - this.drag.offsetLeft;
		this.y = E.clientY - this.drag.offsetTop;
		hsg.addEvent(document,this.Move,"mousemove");
		hsg.addEvent(document,this.Stop,"mouseup");
		this.startFunc.call(this,E, this.container);
		if(document.all){
			hsg.addEvent(this.drag, this.Stop, "losecapture");
			this.drag.setCapture();
			E.cancelBubble = true;
		}else{
			hsg.addEvent(window, this.Stop, "blur");
			E.preventDefault();
		}
	},
	move : function(E){
	...
	stop : function(E){
		hsg.removeEvent(document,this.Move,"mousemove");
		hsg.removeEvent(document,this.Stop,"mouseup");
		this.endFunc.call(this,E, this.container);
		if(document.all){
			hsg.removeEvent(this.drag, this.Stop, "losecapture");
			this.drag.releaseCapture();
			E.cancelBubble = true;
		}else{
			hsg.removeEvent(window, this.Stop, "blur");
		}
	}
};
...
Slider.prototype = {
	...
	init : function(options){
		...
		this.icontainer = hsg.get(options.icontainer);
		this.idrag = hsg.get(options.idrag);
		if(options.plusBtn){
			this.plusBtn = hsg.get(options.plusBtn);
			this.reduceBtn = hsg.get(options.reduceBtn);
		}
		this.direction = options.direction;
		if(options.content){
			this.content = hsg.get(options.content);
			this.panel = hsg.get(options.panel);
	...
	addEvent : function(){
		...
		self.fixedBug();
		if(self.plusBtn){
			hsg.addEvent(self.plusBtn, function(){
				self.goToward("up", self);
			}, "mousedown");
			hsg.addEvent(self.plusBtn, function(){
				clearTimeout(self.timer);
			}, "mouseup");
			hsg.addEvent(self.reduceBtn, function(){
		...
		hsg.addEvent(self.icontainer, function(event){
		...
		new Util.Drag(self.icontainer, self.idrag, {
			...
			moveFunc : function(E, idrag, icontainer){
				if(self.content){
					self.callback(parseInt(hsg.getStyle(idrag, self.direction)), self.Max[self.direction]);
				}
			},
			endFunc : function(E){
				setTimeout(function(){
	...
	run : function (self){
		...
		var posUp = Math.min(Math.max(parseInt(hsg.getStyle(self.idrag, self.direction)) + acc,self.Min[self.direction]),self.Max[self.direction]);
		if(self.content){
	...
	mouseClick : function(event, self){
		var pos = hsg.getPos(event),
			offset = hsg.getS(self.idrag),
			dis = pos[self.direction == "top" ? "pageY" : "pageX"] - offset[self.direction],
			top = parseInt(hsg.getStyle(self.idrag, self.direction)),
		...
		if(self.content){
			self.callback(parseInt(hsg.getStyle(self.idrag, self.direction)), self.Max[self.direction]);
		}
		hsg.preventDefault(event);
	},
	scrollFunc : function scrollFunc(event, self){
		self.dec = hsg.getDetail(event);
		self.isScroll = true;
		self.run();
		hsg.preventDefault(event);
	},
	keydown : function scrollFunc1(event, self){
		var which = hsg.getWhich(event);
		if(which === 38){
			self.dec = - 10;
		}
	...
	fixedBug : function(){
		...
		if(!document.all){
			hsg.addEvent(this.icontainer, function(){
