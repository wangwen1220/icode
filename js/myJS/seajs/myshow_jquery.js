(function($){
	$(function(){
		...
		// 页面滚动浮动导航智能固顶
		if (!isIE6) $('#header').smartFloat({holder: true});
		...
		$('#add').click(function(){
			...
			if (dialog_id == 'upload-pin-dialog' && isIE) {
				e.stopPropagation();
				return; // 如果是 IE 打开新页面执行批量上传
			}
		...
		// 选择专辑时更换常用标签
		$('#pin-info-form').find('.option').live('click', function(){
			var $this = $(this);
		...
		// 点击标签后，把标签 ID 插入标签框 2012-11-1 星期四 被废
		/*$('#tag-list a').live('click', function(){
			var $ths = $(this);
		...
		// 点击标签后，把标签 ID 插入标签
		$('#tag-list').live('hover', function() {
			$(this).toggleClass('fuck');
		}).find('a').live('click', function() {
			var $ths = $(this),
				text = $ths.text(),
				$tag_container = $("#tag-container"),
				$input = $tag_container.next(':text');

			// 使输入框获取焦点，隐藏提示信息
			$input.focus();

			if($ths.hasClass('selected')){// 如果已被选中
				$ths.removeClass('selected');
				//$tag_container.find('.tag-item:contains("' + text + '")').remove();
				$tag_container.find('.tag-item').filter(function(){
					return $('.tag-text', this).text() == text;
				}).remove();
			} else {
				if($tag_container.find('.tag-item').length >= 5){// 判断标签个数
					//alert('最多只能添加 5 个标签！');
					$.dialog({
						//lock: true,
						id: 'artdia-tag-tip',
					...
					$ths.addClass('selected');
					$tag_container.append('<span class="tag-item"><span class="tag-text">' + text + '</span><a href="javascript:;" class="tag-closer" title="删除">×</a></span>');
				}
			}
		...
		// 标签框中标签个数和长度验证
		$('#pin-info-form').find('input.pin-tags').live('blur', function(){
			var $ths = $(this),
				$tag_container = $ths.prev(".tag-container");

			// 隐藏提示信息
			if ($tag_container.find('.tag-item').length) {
				$ths.next('label').hide();
			}
			$ths.trigger('keyup', ["s"]);

		}).live('keyup', function(e, sendkey){ // 如果此事件弹提示框，会把谷歌浏览器玩死
			var $ths = $(this),
				$dialog = $ths.parents('.dialog'),
				$tag_container = $ths.prev(".tag-container"),
				//$tag_list_a = $dialog.find('.tag-list a'),
				text = $ths.val(),
				//text_format = text.replace(/\s/g, '');// 去除空白字符
				text_format = text.replace(/[^\a-\z\A-\Z0-9\u4E00-\u9FA5]/g, '');// 去除空白字符

				$ths.val(text_format);

			if (e.which == 8 && !text) {// 删除标签
				$tag_container.find('.tag-item:last .tag-closer').click();
			} else if (text_format) {
				if ($tag_container.find('.tag-item').length >= 5) {// 标签个数大于 5
					$.dialog({
						//lock: true,
						id: 'artdia-tag-tip',
						content: '最多只能插入 5 个标签！',
					...
					$ths.val('').focus();
				} else if (e.which == 32 && text.indexOf(' ') != -1 || sendkey == 's') {// 空格写入标签，汉字输入是空格上屏所以要检测下
					// 验证字符个数
					if (text_format.length > 15 && e.which != 8 && e.which != 46) {
						//alert("标签内容最长为 15 个字！");
						$.dialog({
							//lock: true,
							id: 'artdia-tag-tip',
							content: '标签内容最长为 15 个字！',
							icon: 'error',
							time: 2
						});
						$ths.focus();
						...
						// 查找含有相同文本的标签
						$tag_item_same = $tag_container.find('.tag-item').filter(function(){
							return $('.tag-text', this).text() == text_format;
						});

						if (!$tag_item_same.length) { // 如果相同文本的标签不存在，则插入
							$tag_container.append('<span class="tag-item"><span class="tag-text">' + text_format + '</span><a href="javascript:;" class="tag-closer" title="删除">×</a></span>');
							if ($ths.is('#pin-tags')) {
								$("#tag-list").find('a').filter(function(){
									return $(this).text() == text_format;
								}).addClass('selected');
							}
						}
					}
		...
		// 删除标签按钮动作
		$('#pin-info-form').find('.tag-closer').live('click', function(){
			var $ths = $(this),
				text = $ths.prev().text(),
				$tag_container = $ths.parents('.tag-container');

			if ($tag_container.is('#tag-container')) {
				// 删除常用标签列表中相应标签的选中状态
				$("#tag-list").find('a').filter(function(){
			...
			// 删除标签
				$ths.parent().remove();
				// 触发输入框事件，同步标签，!!要放在删除后面
				$tag_container.next().keyup().focus();
			return false;
		});

		// 输入框获取焦点
		$('#pin-info-form').find('.tags').live('click', function(){
			$(this).find(':text')[0].focus(); // 这里必须得用原生对象
		});
			...
			if ($ths.hasClass('pin-info-form-batch')) {
				...
				$ths.find('.pin-item').each(function() {
					var $self = $(this),
						...
						//info_obj = $.parseJSON('{"img": "'+img+'", "desc": "'+desc+'"}'),
						info_obj = {},
						//tag_input = $.trim($self.find('.pin-tags').val()),
						tags;
					...
					// 标签文本转为数组
					tags = $self.find('.tag-text').map(function() {
						return $(this).text();
					}).get();

					// 验证标签 todo
					//if (tags.length < 5) {
						//if (tag_input.length > 15){
							//alert('标签最长为 15 个字！');
							//$.dialog({
								//lock: true,
								//id: 'artdia-tag-tip',
							...
							//return false;
						//}
						// 如果输入框中的标签不在标签数组中，则加入到标签数组中
						//if (tag_input && $.inArray(tag_input, tags) == -1) tags.push(tag_input);
					//}

					info_obj.img = img;
					info_obj.desc = desc;
					info_obj.tags = tags;// 操蛋，直接写到 parseJSON 里会转成字符串 todo
					info.push(info_obj);
				});
				...
				var desc = $.trim($ths.find('textarea').val()),
					//tag_input = $.trim($('#pin-tags', this).val()),
					tags;
				...
				// 标签文本转为数组
				tags = $ths.find('.tag-text').map(function() {
					return $(this).text();
				}).get();// || [];

				// 提交
				$.post(url + '&' + data, {'input_tags_arr': tags}, function(d){
					if(d == 0){
						alert('抱歉采集提交出错，请重新采集！');
					}else{
						$dialog.remove();
		...
		$('#upload-dialog-closer').die('click').on('click', function(){
			window.location = document.referrer || "/index.php?m=community&c=index&a=index";
		});
		...
		// 批量上传页
		// 方式一中的输入内容同步到方式二中
		$('#pin-info-way-one').find('.desc').live('keyup', function() {
			$('#pin-info-way-two').find('.desc').val(this.value).blur();
		});
		// 方式一中的输入的标签同步到方式二中
		$('#pin-info-way-one').find('#pin-tags').live('keyup', function() {
			var $ths = $(this),
				html = $ths.prev(".tag-container").html(),
				$tag_container = $('#pin-info-way-two').find(".tag-container"),
				$tags_input = $('#pin-info-way-two').find('.pin-tags');

			$tag_container.html(html);
			$tags_input.val(this.value).blur();
		});
		// 删除该图片
		$('#pin-info-way-two').find('.del-pin').live('click', function(){
			$(this).parents('.progressWrapper').fadeOut(300, function(){
				if (!$(this).siblings('.progressWrapper').length) {
					$('.pin-info-way').hide();
					$('#js-submit').hide();
					$('#notice-wrapper').show();
					//$("#pin-info-form")[0].reset();
					//$('#tag-container').html('');
					$('#upload-pin-dialog').css('top', '20%');
				}
				$(this).remove();
			});
		});
		// 显示常用标签
		$('#pin-info-way-two').find('.pin-item-info .pin-tags').live('focus', function(){
			$(this).siblings('.select-tags').show();
		});
		// 隐藏常用标签
		$('#pin-info-way-two').find('.tags-close').live('click', function(){
		...
		$('#upload-pin-form').live('submit', function(){
			...
			$(this).ajaxSubmit({
				...
				success: function(){
					...
					// 移动弹窗到
					/*$('#upload-pin-dialog').appendTo($page_overlay).css({
						'position': 'absolute',
						'top': '0'
					});
			...
			if($dialog.length){
				...
				$.get(dialog_url, function(d){
					...
					$dialog.appendTo('body').show(function(){
						if (dialogId == 'upload-pin-dialog') {// 如果是批量上传弹窗
							initDivselect();
							$.getScript('/mybatchup/swfupload/swfupload.js', function() {
								$.getScript('/mybatchup/js/swfupload.queue.js', function() {
									$.getScript('/mybatchup/js/fileprogress.js', function() {
										$.getScript('/mybatchup/js/handlers.js');
									});
								});
							});
							...
							// 加载批量上传弹窗插件代码
							/*$.plugin('batchUpload', {
								files: ['/mybatchup/css/default.css','/mybatchup/swfupload/swfupload.js','/mybatchup/js/swfupload.queue.js','/mybatchup/js/fileprogress.js','/mybatchup/js/handlers.js'],
								selectors: ['body'],
								callback : function() {
									alert('over');
									$('#upload-pin-dialog').batchUpload();
								}
							});
							$.plugin('batchUpload').get();*/
							// 隐藏上传选择弹窗
							//$('#upload-select-dialog').find('.upload-closer').click();
							// 移动弹窗到
							$('#upload-pin-dialog').appendTo($page_overlay).css({
	...
	$(window).load(function() {
		...
		// 定位当前分享的缩略图，并加上红色边框
		//$('#thumb-' + $('#pin-img').attr('data-id')).css('border', '2px solid #DC413D');
		//alert($('#thumb-' + $('#pin-img').attr('data-id')).index());

		// 详细页图片滚动
		var $pin_slide = $('#pin-slide');
		var start_index = $('#thumb-' + $('#pin-img').attr('data-id')).css('border-color', '#DC413D').index();
		if($pin_slide.length){
			$pin_slide.jcarousel({
				scroll: 10,
				start: start_index,
				initCallback: function() {
					//alert($('pin-slide').length);
				}
			});
		}
		...
		// 设置 IE6 下 pin-nav-btn 100% 高度无效的问题
		if (isIE6) {
			$('#js-pin-show').find('.pin-nav-btn').height($('#js-pin-show').find('.pin-cnt').height());
		}

		if ($.browser.opera) {
			$('#js-pin-show').find('.pin-nav-btn').addClass('opera');
		}
		...
		function getPin(url){
			$.get(url, function(data){
				var $data = $(data),
					$pin_cnt = $data.find('#pin-slide').nextUntil('#js-stats'),// 获取图片导航后的元素
					$pin_stats = $data.find('#js-stats-item'),// 获取图片导航后的元素
					pin_comments_str = $data.find('#js-pin-comments').html(),// 获取评论的 HTML
					pager_str = $data.find('#pager').html(),// 获取评论翻页的 HTML
					$stats = $('#js-stats'),
					$jiathis = $('#js-jiathis');// 分享到，重新加载会失效
				//$('#main').find('.jcarousel-skin-myshow').nextUntil($stats).remove().end().after($pin_cnt);
				$stats.prevUntil('.jcarousel-skin-myshow').remove().end().before($pin_cnt);
				// 加载转藏、评论等
				$jiathis.nextAll().remove().end().after($pin_stats);
		...
		bd.src = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'hm.baidu.com/h.js?82b22a65dfbeae6f241edda7d3f322af';
		document.body.appendChild(bd);*/

		// 加载 CNZZ 统计代码
		var cnzz = document.createElement('script');
		cnzz.src = 'http://s14.cnzz.com/stat.php?id=4719182&web_id=4719182';
